...
export class LandHPAPathFinder {
  ...
  static build(miniMap: GameMap, options?: LandHPAOptions): LandHPAPathFinder {
    return new LandHPAPathFinder(miniMap, options);
  }

  /**
   * Find a land-only path on the mini-map.
   *
  ...
  /**
   * Convert a mini-map path to a full-res game-map waypoint list, attempting to keep points on land.
   */
  miniPathToGameMap(
    gameMap: GameMap,
    miniPath: TileRef[],
    ensureLand: (tile: TileRef) => boolean,
  ): TileRef[] {
    const full: TileRef[] = [];
    for (const mt of miniPath) {
      const mapped = this.miniTileToGameLandTile(gameMap, mt, ensureLand);
      if (mapped !== null) {
        ...
        if (cx + 1 < this.clusterCols) {
          ...
          if (xA >= 0 && xB < this.miniMap.width()) {
            ...
            for (let y = y0; y <= y1; y++) {
              ...
              const ok = this.miniMap.isLand(a) && this.miniMap.isLand(b);
              if (ok && runStart === null) {
                runStart = y;
              }
              const isEnd = y === y1;
  ...
  private bfsDistancesWithinBounds(
    ...
    if (!this.miniMap.isLand(startTile)) {
      return dist;
    }
    ...
    while (qh < qt) {
      ...
      const tryPush = (nx: number, ny: number) => {
        ...
        if (!this.miniMap.isLand(tile)) return;
  ...
  private bfsPathWithinBounds(
    ...
    if (!this.miniMap.isLand(startTile) || !this.miniMap.isLand(goalTile)) {
      return null;
    }

    const dist = new Int16Array(bounds.width * bounds.height);
    const prev = new Int32Array(bounds.width * bounds.height);
    dist.fill(-1);
    prev.fill(-1);
  ...
  private miniTileToGameLandTile(
    gameMap: GameMap,
    miniTile: TileRef,
    ensureLand: (tile: TileRef) => boolean,
  ): TileRef | null {
